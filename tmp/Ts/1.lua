...
function main()
    ...
    local deviceOffsetHours = os.date("*t").hour - os.date("!%H", os.time())
    local vietnamOffset = (7 - deviceOffsetHours) * 3600  
    local vietnamTime = deviceTime + vietnamOffset
    local TimeClear = tonumber(os.date("%H", vietnamTime))
    local StatusClear = readState()
    if TimeClear%3 ~= 0 then
        writeState(false)
    end
    ...
    if CheckOpenApp[1] ~= -1  then
        ...
        if Manage_Mail[1] ~= -1 then
            ...
            if CheckMail[1] ~= -1 then
                ...
                if CheckFullMail[1] == -1 then
                    if TimeClear%3 == 0 and not StatusClear then
                        writeState(true)
