...
setVolumeLevel(0)
    ...
    local FullID = readFile(userPath() .. luaAliasld.txt)
    ...
    function readState()
        local file = io.open(userPath() .. luastatefile.txt, r)
        if file then
            local state = fileread(a)
            fileclose()
    ...
    function writeState(state)
        local file = io.open(userPath() .. luastatefile.txt, w)
        filewrite(tostring(state))
        fileclose()
    end
    
    function readSave()
        local file = io.open(userPath() .. luasave.txt, r)
        if file then
            local state = fileread(a)
            fileclose()
            return state == true
    ...
    function SearchVideo(str)
        if (str) then
            str = string.gsub(str, n, rn)
            str = string.gsub(str, ([^%w %-%_%.%~]),
                function(c) return string.format(%%%02X, string.byte(c)) end)
            str = string.gsub(str,  , +)
            openURL(youtubewww.youtube.comresultssearch_query= .. str)
            return true
        end
        return false
    end
    ...
    function GetProxy()
        local url = httpsytb.ishoperp.comapiappytbsharenext-detail..ID_Proxy
        local response = {}
        for i=1,20 do
            toast(Get Proxy)
    ...
    function GetVideo()
        local url = httpsytb.ishoperp.comapiappytbsharenext-detail..ID_KeyName
        local response = {}
        for i=1,20 do
    ...
    function GetDomain()
        local url = httpsytb.ishoperp.comapiappytbsharenext-detail6666
        local response = {}
        for i=1,20 do
            toast(Get Domain)
            mSleep(math.random(1000,2200))
...
function clear(path)
    function getAllFilesInDirectory(directory)
        local files = {}
        local command = 'find ' .. directory .. ' -type f 2devnull' 
        local handle = io.popen(command)
        if handle then
            for file in handlelines() do
    ...
    function getAllDirectories(directory)
        local directories = {}
        local command = 'find ' .. directory .. ' -mindepth 1 -type d 2devnull  sort -r' 
        local handle = io.popen(command)
        if handle then
            for dir in handlelines() do
    ...
    function deleteFilesAndDirectories(directory)
        local command = 'rm -rf ' .. directory .. ' 2devnull'
        os.execute(command)
    end
...
function WIPEAPP()
    ...
    os.execute(su mobile -c uicache);
        ...
        io.popen(echo alpine  sudo -u root -S rm -rf varMobileSoftwareUpdate);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryLogs.log);
        io.popen(echo alpine  sudo -u root -S rm -rf varlogs.log);
        io.popen(echo alpine  sudo -u root -S rm -rf varlogsAppleSupport.log);
        io.popen(echo alpine  sudo -u root -S rm -rf varlogsCrashReporter);
        io.popen(echo alpine  sudo -u root -S rm -rf varroot.bash_history);
        io.popen(echo alpine  sudo -u root -S rm -rf vardbdiagnosticsshutdown.log);
        io.popen(echo alpine  sudo -u root -S rm -rf varrootLibraryLogs);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileDownloads);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryCachesSnapshots);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryCachescom.apple.Safari.SafeBrowsing);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryCachescom.apple.WebKit.Networking);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryCachescom.apple.WebKit.WebContent);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibrarycom.apple.WebKit);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryCrashReporter);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryLogsAppleSupport);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryLogsCrashReporter);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryRecents);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibrarySafari);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibrarySafariSafeBrowsing);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryWebClips);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileLibraryWebKit);
        io.popen(echo alpine  sudo -u root -S rm -rf varmobileMediaDownloads);
        io.popen(echo alpine  sudo -u root -S rm -rf privatevarmobileLibraryCachescom.apple.mobilesafari);
        io.popen(echo alpine  sudo -u root -S rm -rf privatevarmobileLibraryCachesSafari);
        io.popen(echo alpine  sudo -u root -S rm privatevarmobileLibraryCookiesCookies.binarycookies);
        io.popen(echo alpine  sudo -u root -S rm privatevarrootLibraryCookiesCookies.binarycookies);
        io.popen(echo alpine  sudo -u root -S killall -9 pasted);
        io.popen(echo alpine  sudo -u root -S killall -9 pasteboardd);
        io.popen(echo alpine  sudo -u root -S launchctl unload -w SystemLibraryLaunchDaemonscom.apple.UIKit.pasteboardd.plist);
        io.popen(echo alpine  sudo -u root -S launchctl unload -w SystemLibraryLaunchDaemonscom.apple.pasteboard.pasted.plist);
	os.execute(rm -rf ~LibraryCachescom.apple.Safari;)
    os.execute(rm -rf ~LibraryCachescom.apple.WebKit.PluginProcess;)
    os.execute(rm -rf ~LibraryCookiesCookies.binarycookies;)
    os.execute(rm -rf ~LibraryPreferencescom.apple.Safari.LSSharedFileList.plist;)
    os.execute(rm -rf ~LibraryPreferencescom.apple.Safari.RSS.plist;)
    os.execute(rm -rf ~LibraryPreferencescom.apple.Safari.plist;)
    os.execute(rm -rf ~LibraryPreferencescom.apple.WebFoundation.plist;)
    os.execute(rm -rf ~LibraryPreferencescom.apple.WebKit.PluginHost.plist;)
    os.execute(rm -rf ~LibraryPreferencescom.apple.WebKit.PluginProcess.plist;)
    os.execute(rm -rf ~LibraryPubSubDatabase;)
    os.execute(rm -rf ~LibrarySavedApplication Statecom.apple.Safari.savedState)
	os.execute(rm -rf privatevarmobileLibrarySafari)
    os.execute(rm -rf varmobileLibrarySafari)
	os.execute(rm -rf varmobileLibrarycom.apple.nsurlsessiond-launchd)
	os.execute(rm -rf varmobileLibrarycom.apple.nsurlsessiond-launchd)
	os.execute(rm -rf varmobileLibraryCachescom.apple.AppleAccountfsCachedData)
	os.execute(rm -rf varmobileLibraryCaches)
	os.execute(rm -rf varmobileLibraryMail)
	os.execute(rm -rf varmobileLibraryLogs)
	os.execute(rm -rf UserLibraryCaches)
    os.execute(rm -rf privatevartmp);
    mSleep(10)
    os.execute(rm -rf varmobileLibraryConfigurationProfiles)
    os.execute(rm -rf varmobileLibraryCloudKit)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.accountsettings.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.celestial.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.commerce.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.identityservicesd.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.ids.service.com.apple.madrid.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.imagent.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.itunesstored.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.Maps.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.storeagent.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.mobilemeaccounts.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.accountsd.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.security.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.ids.plist)
    os.execute(rm -rf varmobileLibraryPreferencescom.apple.icloud.searchpartyd.plist)
    os.execute(rm -rf privatevarmobileLibraryCachescom.apple.MobileAsset.UserFonts)
    os.execute(rm -rf privatevarmobileLibraryCachescom.apple.icloud.fmipserver)
    os.execute(rm -rf privatevarmobileLibraryCookiescom.apple.appstore.binarycookies)
    os.execute(rm -rf privatevarmobileLibraryPreferencescom.apple.storebookmarks.plist)
    os.execute(rm -rf privatevarmobileLibraryCloudKitCloudKitMetadata)
    os.execute(rm -rf privatevarmobileLibraryCloudKitcom.apple.cloudd.plist)
    os.execute(rm -rf privatevarmobileLibraryPreferencescom.apple.cloudkit.sync.plist)
    os.execute(rm -rf privatevarmobileLibraryPreferencescom.apple.cloudkit.pcs.plist)
    os.execute(rm -rf privatevarmobileLibraryPreferencescom.apple.cloudkit.plist)
    os.execute(rm -rf privatevarmobileLibraryPreferencescom.apple.security.cloudkeychainproxy3.keysToRegister)
    os.execute(rm -rf privatevarmobileLibraryPreferencescom.apple.security.idsidentityd.plist)
    os.execute(rm -rf privatevarmobileLibraryPreferencescom.apple.AppStore.plist)
    os.execute(rm -rf privatevarmobileLibraryPreferencescom.apple.AppleAccountUI.plist)
    os.execute(rm -rf privatevarmobileLibraryPreferencescom.apple.storeServices.plist)
    mSleep(10) 
    os.execute(rm -rf privatevarmobileLibrary.umeng)
    os.execute(rm -rf privatevarmobileLibraryCaches)
    os.execute(rm -rf privatevarmobileLibrarycom.apple.iTunesCloud)
    os.execute(rm -rf privatevarmobileLibrarycom.apple.itunesstored)
    os.execute(rm -rf privatevarmobileLibrarycom.apple.nsurlsessiond)
    os.execute(rm -rf privatevarmobileLibrarycom.apple.nsurlsessiond-launchd)
    os.execute(rm -rf privatevarmobileLibraryPasses)
    os.execute(rm -rf privatevarmobileLibrarySafari)
    ts.dborder(varmobileLibraryAccountsAccounts3.sqlite,DELETE FROM ZACCOUNT);
    ts.dborder(varmobileLibraryAccountsAccounts3.sqlite,DELETE FROM ZACCOUNTPROPERTY);  
    mSleep(5)
    clear('privatevarmobileLibrarySafari')
    mSleep(5)
    clear('privatevardblsdcom.apple.lsdidentifiers.plist')
    clear('privatevarmobileLibraryCachescom.apple.itunesstoredfsCachedData');
    mSleep(5)
    clear('privatevarmobileLibraryWebKit');
    clear('privatevarmobileLibraryCachescom.apple.UIKit.pboard');
    mSleep(5)
    clear(privatevarmobileContainersDataApplication)
    clear(privatevarmobileContainersSharedAppGroup)
    clear(privatevarmobileMediaTouchSpritelog)
    mSleep(5)
end 
